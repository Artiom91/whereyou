...
const transactionDescription = transactions.map((transaction, index, array) => `Transaction № ${index + 1}: ${transaction} was ${Math.abs(transaction) > 0 ? 'deposited' : 'withdrew'} `,
...
const balance = transactions.reduce((acc, item, index, arr) => acc + item, 0);
